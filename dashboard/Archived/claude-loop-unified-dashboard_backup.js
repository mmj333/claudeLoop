...
async function getRecentLogs(maxLines = null, sessionName = null) {
  try {
    ...
    const session = sessionName || 'claude-chat';
    let currentLogPath = path.join(ansiLogDir, `claude_${session}_${today}_current_display.ansi`);
    ...
    try {
      ...
      // If today's file doesn't exist, find the most recent log file
      console.log(`Today's ANSI log file not found (${currentLogPath}), searching for recent files...`);
      
      // Check if ANSI directory exists
      try {
        await fs.access(ansiLogDir);
      } catch (dirError) {
        console.log(`ANSI log directory not found: ${ansiLogDir}`);
        return '';
      }
      
      const files = await fs.readdir(ansiLogDir);
      console.log(`Found ${files.length} files in ${ansiLogDir}:`, files);
      
      const logFiles = files
        .filter(file => file.startsWith('claude_') && file.endsWith('_display.ansi'))
        .map(file => {
          // Match files with session name in them
          const match = file.match(/claude_.*_(\d{4}-\d{2}-\d{2})_current_display\.ansi/);
      ...
      if (logFiles.length > 0) {
        ...
        currentLogPath = path.join(ansiLogDir, recentFile.file);
        console.log(`Using recent log file: ${recentFile.file} (${recentFile.date})`);
  ...
  <div class="container">
    ...
    <div class="horizontal-controls-bar">
      ...
      <div style="display: flex; gap: 5px; align-items: center; margin-left: 20px;">
        <span style="font-size: 11px; color: var(--text-secondary); margin-right: 5px;">Monitor:</span>
        <div id="log-monitor-status" style="display: inline-flex; align-items: center; margin-right: 10px;">
          <div class="status-dot" style="width: 8px; height: 8px; margin-right: 5px;"></div>
          <span id="log-monitor-text" style="font-size: 11px;">Checking...</span>
        </div>
        <button class="compact-button button-success" onclick="controlLogMonitor('start')">
          📝 Start
        </button>
        <button class="compact-button button-danger" onclick="controlLogMonitor('stop')">
          🛑 Stop
        </button>
      </div>
      <div style="display: flex; gap: 5px; align-items: center; margin-left: 20px;">
        <button class="compact-button" style="background: var(--accent);" onclick="openClaudeSession()">
          🌐 Open Claude
        </button>
      </div>
    </div>
        ...
        <div style="margin-bottom: 15px; display: flex; align-items: center; gap: 15px;">
          <div>
            ...
            <select id="session-select" onchange="updateSelectedSession()" style="padding: 6px 10px; border-radius: 4px; background: var(--bg-secondary); color: var(--text-primary); border: 1px solid var(--bg-tertiary);">
              <option value="claude-chat">claude-chat (default)</option>
            </select>
          </div>
          <div id="monitor-status" style="display: flex; align-items: center; gap: 8px;">
            <span style="font-size: 12px; color: var(--text-secondary);">Status:</span>
            <span id="monitor-status-text" style="font-size: 12px;">Checking...</span>
          </div>
        </div>
  ...
  <script>
    ...
    async function controlLogMonitor(action) {
      try {
        ...
        // Include session when starting
        if (action === 'start') {
          payload.session = currentSession || 'claude-chat';
        }
    ...
    async function controlLogMonitor(action) {
      try {
        ...
        setTimeout(updateLogMonitorStatus, 500);
      } catch (error) {
        console.error('Log monitor control error:', error);
      }
    }
    ...
    let currentSession = 'claude-chat'; // Default session
    ...
    async function loadTmuxSessions() {
      try {
        ...
        defaultOption.value = 'claude-chat';
        defaultOption.textContent = 'claude-chat (default)';
        select.appendChild(defaultOption);
        
        // Add other sessions
        if (data.sessions && data.sessions.length > 0) {
          data.sessions.forEach(session => {
            if (session !== 'claude-chat') {
    ...
    async function updateMonitorStatus() {
      try {
        const response = await fetch('/api/log-monitor/status?instance=default');
        const data = await response.json();
        const statusText = document.getElementById('monitor-status-text');
        
        if (data.running) {
          statusText.innerHTML = '<span style="color: var(--success);">✓ Capturing</span>';
        } else {
    ...
    async function updateLogs(isScrollUpdate = false) {
    ...
    async function sendCompactCommand() {
      ...
      try {
        ...
        const response = await fetch('/api/control', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'send-message', message: '/compact' })
        });
