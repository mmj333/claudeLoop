...
class OptimizedClaudeMonitor {
  ...
  async start() {
    ...
    await fsPromises.mkdir(path.join(CONFIG.logDir, 'ANSI_tmp'), { recursive: true });
    ...
    this.currentAnsiLogPath = this.getAnsiLogPath();
    this.lastRotationDate = new Date().toDateString();
    
    await this.initializeLog();
    this.startMonitoring();
    
    console.log(`✅ Monitor started (${CONFIG.sessionName})`);
  }

  getCurrentLogPath() {
    const date = new Date().toISOString().split('T')[0];
    return path.join(CONFIG.logDir, `${CONFIG.sessionName}_${date}.log`);
  }

  getAnsiLogPath() {
    const ansiDir = path.join(CONFIG.logDir, 'ANSI_tmp');
    return path.join(ansiDir, `${CONFIG.sessionName}.log`);
  }
  ...
  async captureContent(lines = CONFIG.maxCaptureLines, includeAnsi = false) {
    const startTime = Date.now();
    
    try {
      // Use -pet for ANSI codes, -pt for plain text
      const flags = includeAnsi ? '-pet' : '-pt';
      const { stdout } = await execPromise(
        `tmux capture-pane ${flags} "${CONFIG.sessionName}" -S -${lines} 2>/dev/null || echo ""`
      );
  ...
  async initializeLog() {
    try {
      ...
      // Initialize ANSI log
      const ansiContent = await this.captureContent(CONFIG.maxCaptureLines, true);
  ...
  async appendNewContent() {
    try {
      ...
      const content = await this.captureContent(captureLines, false);
      if (!content) return;
      
      // Also capture ANSI content for ANSI log
      const ansiContent = await this.captureContent(captureLines, true);
      
      // Efficient append strategy
      if (this.idleState.idleLevel === 0) {
        // Active user: full append logic
        await this.smartAppend(content);
        // Also update ANSI log
        if (ansiContent) {
          await this.updateAnsiLog(ansiContent);
        }
      } else {
        ...
        // Update ANSI log minimally when idle
        if (ansiContent) {
          const ansiLines = ansiContent.split('\n').slice(-20);
          if (ansiLines.length > 0) {
            await fsPromises.appendFile(this.currentAnsiLogPath, '\n' + ansiLines.join('\n'));
          }
        }
      }
  ...
  async updateAnsiLog(ansiContent) {
    try {
      // For ANSI log, just overwrite with latest content
      // This matches the original behavior of the ANSI logs
      await fsPromises.writeFile(this.currentAnsiLogPath, ansiContent);
    } catch (err) {
      console.error('Error updating ANSI log:', err.message);
    }
  }
  ...
  async smartAppend(content) {
    ...
    if (overlapIndex > 0 && overlapIndex < contentLines.length) {
      ...
      if (newLines.length > 0) {
        ...
        // Only log when active and significant changes
        if (this.idleState.idleLevel === 0 && newLines.length > 10) {
  ...
  startMonitoring() {
    this.intervalHandle = setInterval(async () => {
      ...
      // Log performance stats every 500 checks (less frequent)
      if (this.performanceStats.captureCount % 500 === 0 && this.performanceStats.captureCount > 0) {
