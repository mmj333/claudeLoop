...
async function getContextStatus() {
  try {
    // Estimate context from log file - use session-specific log
    const dateStr = new Date().toISOString().split('T')[0].replace(/-/g, '-');
    const currentLog = path.join(CONFIG.logDir, `claude-loop1_${dateStr}.log`);
      ...
      setTimeout(() => {
        ...
        inputs.forEach(input => {
          if (input.type === 'checkbox' || input.type === 'radio') {
            ...
            input.addEventListener('input', () => autoSave(false));
          }
        });
