...
async function scrapeContextFromTmux(session) {
  try {
    ...
    // Split into lines and look for the status line area
    const lines = visibleContent.split('\n');
    
    // Find the input box boundaries (the box with > prompt)
    let inputBoxBottomIndex = -1;
    for (let i = lines.length - 1; i >= 0; i--) {
      // Look for the bottom border of the input box
      if (lines[i].includes('╰─') && lines[i].includes('─╯')) {
        inputBoxBottomIndex = i;
        break;
      }
    }
    
    // If we found the input box, only look at lines below it (the status area)
    let statusLines = [];
    if (inputBoxBottomIndex >= 0) {
      statusLines = lines.slice(inputBoxBottomIndex + 1);
    } else {
      // Fallback: just check the last 5 lines which should be the status area
      statusLines = lines.slice(-5);
    }
    
    // Join the status lines and look for context indicator
    const statusText = statusLines.join(' ');
    
    // Look for context percentage in the status area
    // The format appears to be something like "? for shortcuts    Context: 15%    ◯ IDE connected"
    const contextPatterns = [
      /Context:\s*(\d+)%/i,
      /(\d+)%\s*Context/i,
      // With various separators that might appear
      /\s+Context:\s*(\d+)%/,
      /\│\s*Context:\s*(\d+)%/,
      /•\s*Context:\s*(\d+)%/,
      /◐\s*Context:\s*(\d+)%/,
    ...
    for (const pattern of contextPatterns) {
      ...
      if (match) {
        ...
        console.log(`Found context in status area: ${percentage}%`);
        return percentage;
      }
    }
    
    return null; // No context indicator found in status area
  } catch (e) {
