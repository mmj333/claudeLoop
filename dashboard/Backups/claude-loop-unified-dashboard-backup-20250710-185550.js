...
async function scrapeContextFromTmux(session) {
  try {
    ...
    // Look for lines that come after the input box bottom border (╯)
    const lines = visibleContent.split('\n');
    let afterInputBox = false;
    let contextFound = null;
    
    for (const line of lines) {
      // Once we see the input box bottom border, start looking
      if (line.includes('╯')) {
        afterInputBox = true;
        continue;
      }
      
      // Only look in lines after the input box
      if (afterInputBox) {
        // Simple pattern: find a number followed by % near the word "context" (case insensitive)
        // This handles all variations like "Context: 15%", "15% Context", etc.
        const contextMatch = line.match(/(\d+)%/);
        if (contextMatch && line.toLowerCase().includes('context')) {
          contextFound = parseInt(contextMatch[1]);
          console.log(`Found context in status area: ${contextFound}%`);
          break;
        }
      }
    }
