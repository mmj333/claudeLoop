...
async function scrapeContextFromTmux(session) {
  try {
    ...
    // Look for a percentage number near the word "context" (within ~20 chars)
    // This regex looks for patterns like:
    // - "Context: 15%"
    // - "15% Context" 
    // - "something Context: 15% something"
    // The [\s\S] matches any character including newlines
    const contextMatch = statusAreaText.match(/context[\s\S]{0,20}?(\d+)%|(\d+)%[\s\S]{0,20}?context/i);
    
    if (contextMatch) {
      // Get the captured percentage (could be in group 1 or 2)
      const percentage = parseInt(contextMatch[1] || contextMatch[2]);
