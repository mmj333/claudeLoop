...
async function startLoop(session = 'claude', config = loopConfig) {
  ...
  const loopInterval = setInterval(async () => {
    try {
      ...
      // Get current time for conditional messages
      const now = new Date();
      const hour = now.getHours();
      ...
      // Time-based messages
      if (config.conditionalMessages.morningMessage.enabled && 
          hour >= config.conditionalMessages.morningMessage.startHour && 
          hour < config.conditionalMessages.morningMessage.endHour) {
        message = config.conditionalMessages.morningMessage.message;
      } else if (config.conditionalMessages.afternoonMessage.enabled && 
                 hour >= config.conditionalMessages.afternoonMessage.startHour && 
                 hour < config.conditionalMessages.afternoonMessage.endHour) {
        message = config.conditionalMessages.afternoonMessage.message;
      } else if (config.conditionalMessages.eveningMessage.enabled && 
                 hour >= config.conditionalMessages.eveningMessage.startHour && 
                 hour < config.conditionalMessages.eveningMessage.endHour) {
        message = config.conditionalMessages.eveningMessage.message;
      }
...
<body>
  ...
  <script>
    ...
    function updateUIWithConfig(config) {
      ...
      setElementValue('context-warning', config.contextWarningPercent || 20);
      setElementValue('context-critical', config.contextCriticalPercent || 10);
      setElementValue('use-start-time', config.useStartTime || false, 'checked');
      setElementValue('start-time', config.startTime || '09:00');
      setElementValue('custom-message', config.customMessage || 'Please continue');
      ...
      // Schedule
      if (config.schedule) {
        setElementValue('schedule-enabled', config.schedule.enabled || false, 'checked');
