...
const { getSessionConfig, saveSessionConfig } = require('./config-utils.js');
...
let loopConfig = {
  ...
  // conversationId moved to session-map.json via sessionMatcher
  delayMinutes: 10,
    ...
    switch (pathname) {
      case '/api/config':
        if (method === 'GET') {
          ...
          if (session) {
            // Use helper to get config with conversationId merged
            const config = await getSessionConfig(session, { loopConfig, sessionMatcher });
          ...
          if (data.session) {
            ...
            const { conversationId, ...configWithoutConvId } = data.config || {};
            
            // Save session-specific config using helper (automatically removes conversationId)
            await saveSessionConfig(data.session, configWithoutConvId);
            console.log('Config saved for session: ' + data.session);
            
            // If conversationId was provided, update session-map.json
            if (conversationId) {
              await sessionMatcher.setSessionConversation(data.session, conversationId, process.cwd());
            }
          } else {
            // Save global config (remove conversationId if present)
            const { conversationId, ...dataWithoutConvId } = data;
            loopConfig = { ...loopConfig, ...dataWithoutConvId };
            await saveConfig();
          }
          res.writeHead(200, { 'Content-Type': 'application/json' });
      ...
      case '/api/tmux-sessions':
        ...
        const sessionNames = tmuxSessions.trim().split('\n').filter(s => s);
        
        // Load custom names using helper
        const sessionsWithNames = await Promise.all(sessionNames.map(async (sessionName) => {
          const config = await getSessionConfig(sessionName, { loopConfig, sessionMatcher });
        ...
        res.end(JSON.stringify({ 
          sessions: sessionNames,  // Keep backward compatibility
          sessionsWithNames       // New enhanced format
        }));
        break;
      ...
      case '/api/tmux-pane-cwd':
        ...
        try {
          ...
          // First check if session has a saved conversation ID via sessionMatcher
          const trackedConv = sessionMatcher.getTrackedConversation(cwdSession);
          const savedConversationId = trackedConv ? trackedConv.conversationId : null;
          ...
          console.error('Failed to get initial CWD from conversation:', error);
          res.writeHead(500, { 
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*' 
          });
      ...
      case '/api/conversation/auto-associate':
        if (method === 'POST') {
          ...
          try {
            // Get recent text from tmux (before the prompt)
            const tmuxCmd = `tmux capture-pane -t "${session}" -p -S -100 | grep -B 20 "╭─" | head -n 20`;
            ...
            // Extract a meaningful snippet (first few lines)
            const searchSnippet = recentText.split('\n')
              ...
              try {
                ...
                for (const file of jsonlFiles) {
                  ...
                  if (content.includes(searchSnippet)) {
                    foundConversationId = file.replace('.jsonl', '');
                    
                    // Try to get working directory from this conversation
                    const lines = content.split('\n').slice(0, 10);
                    for (const line of lines) {
                      if (!line.trim()) continue;
                      try {
            ...
            // Save the association (single source of truth)
            await sessionMatcher.setSessionConversation(session, foundConversationId, foundWorkingDir || process.cwd());
      ...
      case '/api/conversation/assign':
        if (method === 'POST') {
          ...
          // Save to session matcher (single source of truth)
          await sessionMatcher.setSessionConversation(session, conversationId, workingDirectory || process.cwd());
          console.log(`Assigned conversation ${conversationId} to ${session}`);
      ...
      case '/api/conversation/resume':
        if (method === 'POST') {
          ...
          try {
            ...
            let workingDir = cwd || '/home/michael/InfiniQuest';
            ...
            if (!cwd || cwd === 'unknown') {
              try {
                ...
                console.warn('Could not determine working directory from conversation file');
              }
            }
            ...
            if (result.resumed) {
              // Update session association first
              if (sessionMatcher) {
              ...
              conversationAutoSetup.monitorAndSetup(session, {
                ...
                if (setupResult.success) {
                  console.log(`[API] Resume fork detected for ${session}: ${setupResult.conversationId} (forked from ${conversationId})`);
                } else {
                  console.warn(`[API] Resume fork detection failed for ${session}: ${setupResult.message}`);
                }
              }).catch(err => {
                console.error(`[API] Resume fork detection error for ${session}:`, err);
              });
            }
      ...
      case '/api/claude/start':
        if (method === 'POST') {
          ...
          const session = data.session || 'claude';
          const config = await getSessionConfig(session, { loopConfig, sessionMatcher });
          const workingDir = config.workingDirectory || '/home/michael/InfiniQuest';
          ...
          if (result.initialized) {
            conversationAutoSetup.monitorAndSetup(session, {
              ...
              if (setupResult.success) {
                console.log(`[API] Auto-setup completed for ${session}: ${setupResult.conversationId}`);
              } else {
                console.warn(`[API] Auto-setup failed for ${session}: ${setupResult.message}`);
              }
            }).catch(err => {
              console.error(`[API] Auto-setup error for ${session}:`, err);
            });
          }
        ...
        if (method === 'POST') {
          ...
          if (result.resumed) {
            conversationAutoSetup.monitorAndSetup(session, {
              ...
              if (setupResult.success) {
                console.log(`[API] Fork auto-setup completed for ${session}: ${setupResult.conversationId} (forked from ${conversationId})`);
              } else {
                console.warn(`[API] Fork auto-setup failed for ${session}: ${setupResult.message}`);
              }
            }).catch(err => {
              console.error(`[API] Fork auto-setup error for ${session}:`, err);
            });
          }
...
async function getContextStatus(sessionName = null) {
  try {
    ...
    try {
      ...
      console.error('Error calculating chars since compact:', e);
    }
...
async function getConditionalMessage(sessionName = null) {
  ...
  const sessionConfig = await getSessionConfig(sessionName, { loopConfig, sessionMatcher });
  const config = sessionConfig.conditionalMessages || loopConfig.conditionalMessages;
  console.log(`Using config for ${sessionName}:`, config.standardMessage);
...
function onCompact(session) {
  ...
  contextState.sessions[session].linesAfterCompact = 0;
  contextState.sessions[session].lastCompactTime = new Date();
  contextState.sessions[session].currentPercent = 100;
  console.log(`[Context] Compact detected for session ${session}`);
}
...
async function getActiveConditionalMessage(session = 'claude') {
  try {
    ...
    console.log(`[Conditional] Session ${session} - Context: ${contextPercent}%, Lines after compact: ${linesAfterCompact}`);
    
    // Priority 1: After Compact message
    if (messages.afterCompactMessage?.enabled && 
        linesAfterCompact <= messages.afterCompactMessage.linesAfterCompact) {
      console.log(`[Conditional] Using after-compact message`);
      return {
        type: 'afterCompact',
    ...
    if (messages.lowContextMessage?.enabled && 
      ...
      if (messages.lowContextMessage.autoFinish) {
        message += '\n\nIMPORTANT: If you\'ve completed all tasks, please say "Finished everything for now!" to pause the loop.';
      }
    ...
    if (messages.morningMessage?.enabled && 
      ...
      console.log(`[Conditional] Using morning message`);
      return {
        type: 'morning',
    ...
    if (messages.afternoonMessage?.enabled && 
      ...
      console.log(`[Conditional] Using afternoon message`);
      return {
        type: 'afternoon',
    ...
    if (messages.eveningMessage?.enabled && 
      ...
      console.log(`[Conditional] Using evening message`);
      return {
        type: 'evening',
    ...
    // Priority 4: Standard message
    if (messages.standardMessage?.enabled) {
      console.log(`[Conditional] Using standard message`);
      return {
        type: 'standard',
    ...
    console.log(`[Conditional] No conditional message active`);
    return null;
    
  } catch (error) {
    console.error('[Conditional] Error getting active message:', error);
    return null;
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/dashboard-schedule.js') {
    ...
    fs.readFile(schedulePath, 'utf8')
      ...
      .catch(err => {
        console.error('Error serving dashboard-schedule.js:', err);
        res.writeHead(404);
        res.end('Not found');
      });
    return;
  ...
  if (pathname === '/dashboard-conditional.js') {
    ...
    fs.readFile(conditionalPath, 'utf8')
      ...
      .catch(err => {
        console.error('Error serving dashboard-conditional.js:', err);
        res.writeHead(404);
        res.end('Not found');
      });
    return;
  ...
  if (pathname === '/dashboard-conversations.js') {
    ...
    fs.readFile(conversationsPath, 'utf8')
      ...
      .catch(err => {
        console.error('Error serving dashboard-conversations.js:', err);
        res.writeHead(404);
        res.end('Not found');
      });
    return;
  ...
  if (pathname === '/session-state.js') {
    ...
    fs.readFile(sessionStatePath, 'utf8')
      ...
      .catch(err => {
        console.error('Error serving session-state.js:', err);
        res.writeHead(404);
        res.end('Not found');
      });
    return;
...
loadConfig().then(async () => {
  ...
  for (const [session, info] of Object.entries(activeLoops)) {
    if (info.active) {
      ...
      try {
        // Load session-specific config using helper
        const sessionConfig = await getSessionConfig(session, { loopConfig, sessionMatcher });
