...
const { getSessionConfig, saveSessionConfig } = require('./config-utils');

class SimpleClaudeSessionTracker {
    constructor() {
        this.claudeConfigFile = path.join(os.homedir(), '.claude.json');
        // No more session map file - using config files as source of truth
        this.loopConfig = {};
    }

    async init(loopConfig = {}) {
        // Store loop config for use in other methods
        this.loopConfig = loopConfig;
        // No session map to load - configs are loaded on demand
    }

    // Deprecated - configs are saved individually
    async saveSessionMap() {
        // No-op for backward compatibility
        console.log('[SessionTracker] saveSessionMap called but using config files now');
    }
    ...
    async trackConversation(sessionName, conversationId) {
        ...
        if (conv) {
            // Save to config file (single source of truth)
            const config = await getSessionConfig(sessionName, { loopConfig: this.loopConfig });
            await saveSessionConfig(sessionName, {
                ...config,
                conversationId: conv.id,
                conversationTitle: conv.title,
                workingDirectory: this.projectPathToDirectory(conv.projectPath),
    ...
    async trackActiveSession(sessionName) {
        ...
        await saveSessionConfig(sessionName, {
            ...
            workingDirectory: this.projectPathToDirectory(latest.projectPath),
            trackedAt: new Date().toISOString()
        });
    ...
    async getTrackedConversation(sessionName) {
        ...
        if (config.conversationId) {
            return {
                ...
                projectPath: config.workingDirectory ? this.getProjectPath(config.workingDirectory) : '',
                trackedAt: config.trackedAt
            };
        }
        return null;
    }
