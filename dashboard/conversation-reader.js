...
class ConversationReader {
    ...
    /**
     * Get the latest N messages from a conversation (optimized)
     * @param {string} conversationId - The conversation ID
     * @param {number} count - Number of messages to return
     * @returns {Promise<Array>} Array of message objects
     */
    async getLatestMessages(conversationId, count = 50) {
        // Optimization: Read file backwards to get only last N lines
        // instead of reading entire file
        try {
            const filePath = await this.findConversationFile(conversationId);
            if (!filePath) {
                console.error('Conversation file not found:', conversationId);
                return [];
            }

            // Use tail-like approach to read only last part of file
            const { exec } = require('child_process');
            const util = require('util');
            const execAsync = util.promisify(exec);
            
            // Get last N*3 lines (to account for multi-line messages) 
            // and parse them
            const tailCount = count * 3;
            const { stdout } = await execAsync(`tail -n ${tailCount} "${filePath}"`, {
            ...
            for (const line of lines) {
                try {
                    const data = JSON.parse(line);
                    const message = this.processMessage(data, messages.length);
                    if (message) {
                        messages.push(message);
                    }
                } catch (e) {
    ...
    processMessage(data, index) {
        ...
        } else if (data.content && typeof data.content === 'object' && data.content.role) {
            type = data.content.role;
        }
        
        // Use recursive extraction for content
        let messageText = '';
        
        // Try content field first
        if (data.content) {
            messageText = this.extractTextRecursive(data.content);
        }
        
        // Try message field if content didn't work
        if (!messageText && data.message) {
            messageText = this.extractTextRecursive(data.message);
        }
        
        // Try text field
        if (!messageText && data.text) {
            messageText = this.extractTextRecursive(data.text);
        }
        
        // Only return if we found some text
        if (messageText) {
            return {
                id: data.uuid || data.id || data.leafUuid || `msg-${index}`,
                type: type,
                content: messageText,
                timestamp: data.timestamp || new Date().toISOString(),
                metadata: {
