...
const dashboardAPI = {
  ...
  async saveConfig(config, session = null) {
    // If session is provided, wrap config properly for the API
    if (session) {
      return this.post('/api/config', { config, session });
    }
    // For global config, spread it directly
  ...
  async getTmuxTail(lines = 10, session = null, currentTab = 'tmux') {
    const url = session
      ? `/api/tmux-tail?lines=${lines}&session=${session}&tab=${currentTab}`
      : `/api/tmux-tail?lines=${lines}&tab=${currentTab}`;
    return this.request(url);
  },
  ...
  // Conversation APIs
  async scanConversations(full = false, cacheOnly = false) {
    return this.request(`/api/conversation/scan?full=${full}&cacheOnly=${cacheOnly}`);
  },
  ...
  async getConversationMessages(conversationId, limit = 100) {
    return this.request(`/api/conversation/messages?id=${conversationId}&limit=${limit}`);
  },
  ...
  async getCurrentConversation(session) {
    return this.request(`/api/conversation/current${session ? `?session=${session}` : ''}`);
  },
