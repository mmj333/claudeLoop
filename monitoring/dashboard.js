...
const CONFIG = {
  ...
  logDir: process.env.LOG_DIR || path.join(__dirname, '../tmp/claudeLogs'),
  sessionDir: process.env.SESSION_DIR || path.join(__dirname, '../tmp/session_summaries'),
  maxLogLines: 100,
};
...
async function getLoopStatus() {
  try {
    ...
    let processes = [];
    try {
      processes = execSync('ps aux | grep -E "claude-loop|claude loop" | grep -v grep', { encoding: 'utf-8' })
        .trim()
        .split('\n')
...
async function getLogFiles() {
  try {
    ...
    const fileStats = await Promise.all(
      logFiles.slice(0, 10).map(async (file) => {
...
<body>
  <div class="container">
    ...
    <div class="tabs">
      ...
      <div class="tab" onclick="showTab('commands')">🛠️ Commands</div>
    </div>
    ...
    <div id="commands-tab" class="tab-content">
      ...
      <div class="command-box">
        <h4>Start Claude Loop:</h4>
        <div class="command">cd /home/michael/InfiniQuest && ./tmp/claudeLoop/claude-loop-enhanced-v2.sh</div>
        
        <h4>Monitor Logs:</h4>
        <div class="command">tail -f tmp/claudeLogs/claude_$(date +%Y-%m-%d)_current.txt</div>
        
        <h4>Check Process:</h4>
        <div class="command">ps aux | grep -E "claude-loop|claude loop" | grep -v grep</div>
        
        <h4>Stop Claude Loop:</h4>
        <div class="command">pkill -f "claude-loop"</div>
      </div>
    </div>
  ...
  <script>
    ...
    async function loadLog(filename) {
      try {
        ...
        document.getElementById('current-log').textContent = data.content || 'Empty log file';
        ...
        document.getElementById('current-log').textContent = 'Error loading log: ' + error.message;
      }
    }
    ...
    async function loadSummary(filename) {
      try {
        ...
        const data = await response.json();
        document.getElementById('current-log').textContent = data.content || 'Empty summary';
        ...
        document.getElementById('current-log').textContent = 'Error loading summary: ' + error.message;
      }
    }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (pathname === '/') {
      ...
      // Get log content
      const filename = decodeURIComponent(pathname.replace('/api/log/', ''));
      const filePath = path.join(CONFIG.logDir, filename);
      
      // Security check
      if (!filePath.startsWith(CONFIG.logDir) || filename.includes('..')) {
        res.writeHead(403);
        res.end(JSON.stringify({ error: 'Access denied' }));
        return;
      }
      
      const content = await getLogTail(filePath);
      res.writeHead(200);
      res.end(JSON.stringify({ content }));
      
    } else if (pathname.startsWith('/api/summary/')) {
      // Get summary content
      const filename = decodeURIComponent(pathname.replace('/api/summary/', ''));
      const filePath = path.join(CONFIG.sessionDir, filename);
      
      // Security check
      if (!filePath.startsWith(CONFIG.sessionDir) || filename.includes('..')) {
        res.writeHead(403);
        res.end(JSON.stringify({ error: 'Access denied' }));
        return;
      }
